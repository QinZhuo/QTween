...
public class Test : MonoBehaviour
{
    ...
    public void RunTween()
    {
        ...
        if (tween == null)
        {
            tween = cube.transform.QMove( new Vector3(0, 3, 0) , 1).Start(Vector3.zero).AutoDestory(false)
                    .Next(cube.transform.QRotate(new Vector3(70, 167, 34) , 1)).Start(Quaternion.identity).AutoDestory(false).Play(to);
        }
        else
        {
            tween.Play(to);
